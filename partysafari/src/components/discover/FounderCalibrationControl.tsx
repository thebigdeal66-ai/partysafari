...
/**
 * Internal calibration tooling — Accurate / Inaccurate / short note, attached to
 * a recommendation that is already on screen.
 *
 * **Absent, not hidden, for everyone else.** An empty `targets` list returns
 * `null`, so a non-approved account renders no wrapper, no label and no buttons:
 * there is nothing in the tree to unhide with a devtools class toggle, and the
 * surrounding layout is byte-identical to what it was before this sprint. The
 * approval decision itself lives in `featureFlags.isApprovedTester`; this
 * component only reflects it.
 *
 * It is styled to read as internal: dashed border, muted palette, an explicit
 * "Founder calibration" label. It is not an admin console and should never grow
 * into one.
 *
 * Presentational and Supabase-free on purpose. The write is injected as
 * `onSubmit` by the surface that owns the data, which keeps the component
 * renderable in a plain unit test.
 */

export type FounderCalibrationTarget = {
  feature: CalibrationFeature;
  /** Short word for what is being judged, e.g. "Card" or "Pulse". */
  label: string;
};
...
export type FounderCalibrationControlProps = {
  /** One row per judgeable feature. Empty means the viewer is not an approved tester. */
